"""
Mississauga ArcGIS Provider Module

Queries City of Mississauga Open Data via ArcGIS REST services for:
- Zoning by-law designations (By-law 0225-2007)
- Official Plan land use designations
- Secondary Plan areas
- Ward boundaries
- Neighbourhood boundaries

Mississauga uses a comprehensive ArcGIS-based Open Data platform.
"""

import time
from typing import Dict, Any, Optional, List

import httpx

from .base import BaseProvider, ProviderResult


class MississaugaArcGISProvider(BaseProvider):
    """
    Query Mississauga municipal data via ArcGIS REST services.

    Uses City of Mississauga Open Data Portal endpoints.
    """

    name = "Mississauga Open Data"
    base_url = "https://services6.arcgis.com/hCmfchgP1QuP3nCz/ArcGIS/rest/services"
    rate_limit = 2.0  # 2 requests per second
    cache_ttl = 86400 * 7  # 7 days

    # Layer endpoints
    ZONING_URL = f"{base_url}/Zoning/FeatureServer/0/query"
    OFFICIAL_PLAN_URL = f"{base_url}/Official_Plan_Land_Use_Designations/FeatureServer/0/query"
    SECONDARY_PLAN_URL = f"{base_url}/Secondary_Plans/FeatureServer/0/query"
    WARDS_URL = f"{base_url}/Ward_Boundaries/FeatureServer/0/query"
    NEIGHBOURHOODS_URL = f"{base_url}/Neighbourhood_Boundaries/FeatureServer/0/query"

    # Zoning category mappings for Mississauga By-law 0225-2007
    ZONE_CATEGORIES = {
        "R": "Residential",
        "RM": "Residential Multiple",
        "RA": "Residential Apartment",
        "C": "Commercial",
        "CC": "City Centre Commercial",
        "E": "Employment",
        "G": "Greenlands",
        "OS": "Open Space",
        "I": "Institutional",
        "U": "Utility",
        "D": "Development",
        "H": "Highway Commercial",
        "P": "Parkway Belt",
    }

    async def query(
        self,
        lat: float,
        lon: float,
        radius_m: int = 50,
        **kwargs,
    ) -> ProviderResult:
        """
        Query Mississauga municipal data for location.

        Args:
            lat: Latitude (WGS84)
            lon: Longitude (WGS84)
            radius_m: Search radius (default 50m for point lookup)

        Returns:
            ProviderResult with zoning and planning data
        """
        start_time = time.time()
        warnings = []

        results = {
            "zoning_designation": None,
            "zoning_category": None,
            "permitted_uses": [],
            "official_plan_designation": None,
            "official_plan_policies": [],
            "secondary_plan": None,
            "secondary_plan_policies": [],
            "ward": None,
            "ward_name": None,
            "neighbourhood_name": None,
            "neighbourhood_id": None,
        }

        try:
            # Query all layers in parallel
            zoning_data = await self._query_zoning(lat, lon)
            op_data = await self._query_official_plan(lat, lon)
            secondary_data = await self._query_secondary_plan(lat, lon)
            ward_data = await self._query_ward(lat, lon)
            neighbourhood_data = await self._query_neighbourhood(lat, lon)

            # Merge zoning results
            if zoning_data:
                results["zoning_designation"] = zoning_data.get("zone_code")
                results["zoning_category"] = zoning_data.get("zone_category")
                results["permitted_uses"] = zoning_data.get("permitted_uses", [])
                results["zoning_exception"] = zoning_data.get("exception")

            # Merge Official Plan results
            if op_data:
                results["official_plan_designation"] = op_data.get("designation")
                results["official_plan_policies"] = op_data.get("policies", [])

            # Merge Secondary Plan results
            if secondary_data:
                results["secondary_plan"] = secondary_data.get("plan_name")
                results["secondary_plan_policies"] = secondary_data.get("policies", [])

            # Merge ward results
            if ward_data:
                results["ward"] = ward_data.get("ward_number")
                results["ward_name"] = ward_data.get("ward_name")

            # Merge neighbourhood results
            if neighbourhood_data:
                results["neighbourhood_name"] = neighbourhood_data.get("name")
                results["neighbourhood_id"] = neighbourhood_data.get("id")

            response_time = (time.time() - start_time) * 1000

            return self._make_result(
                success=True,
                data=results,
                response_time_ms=response_time,
                warnings=warnings,
                metadata={
                    "source_url": "https://data.mississauga.ca",
                    "governing_bylaw": "0225-2007 (Mississauga)",
                },
            )

        except Exception as e:
            response_time = (time.time() - start_time) * 1000
            return self._make_result(
                success=False,
                error=str(e),
                response_time_ms=response_time,
            )

    async def _query_zoning(self, lat: float, lon: float) -> Optional[Dict[str, Any]]:
        """Query Mississauga zoning layer."""
        try:
            params = {
                "geometry": f"{lon},{lat}",
                "geometryType": "esriGeometryPoint",
                "inSR": "4326",
                "spatialRel": "esriSpatialRelIntersects",
                "outFields": "*",
                "returnGeometry": "false",
                "f": "json",
            }

            async with httpx.AsyncClient() as client:
                response = await client.get(
                    self.ZONING_URL,
                    params=params,
                    timeout=15.0,
                    headers={"User-Agent": "LocationOverview/1.0"},
                )

                if response.status_code == 200:
                    data = response.json()
                    features = data.get("features", [])

                    if features:
                        attrs = features[0].get("attributes", {})
                        zone_code = attrs.get("ZONE_CODE") or attrs.get("ZONING")

                        return {
                            "zone_code": zone_code,
                            "zone_category": self._categorize_zone(zone_code),
                            "exception": attrs.get("EXCEPTION_NO"),
                            "permitted_uses": self._get_permitted_uses(zone_code),
                        }

        except Exception:
            pass

        return None

    async def _query_official_plan(
        self,
        lat: float,
        lon: float,
    ) -> Optional[Dict[str, Any]]:
        """Query Mississauga Official Plan layer."""
        try:
            params = {
                "geometry": f"{lon},{lat}",
                "geometryType": "esriGeometryPoint",
                "inSR": "4326",
                "spatialRel": "esriSpatialRelIntersects",
                "outFields": "*",
                "returnGeometry": "false",
                "f": "json",
            }

            async with httpx.AsyncClient() as client:
                response = await client.get(
                    self.OFFICIAL_PLAN_URL,
                    params=params,
                    timeout=15.0,
                    headers={"User-Agent": "LocationOverview/1.0"},
                )

                if response.status_code == 200:
                    data = response.json()
                    features = data.get("features", [])

                    if features:
                        attrs = features[0].get("attributes", {})
                        designation = attrs.get("LAND_USE") or attrs.get("DESIGNATION")

                        return {
                            "designation": designation,
                            "policies": self._get_op_policies(designation),
                        }

        except Exception:
            pass

        return None

    async def _query_secondary_plan(
        self,
        lat: float,
        lon: float,
    ) -> Optional[Dict[str, Any]]:
        """Query Mississauga Secondary Plan layer."""
        try:
            params = {
                "geometry": f"{lon},{lat}",
                "geometryType": "esriGeometryPoint",
                "inSR": "4326",
                "spatialRel": "esriSpatialRelIntersects",
                "outFields": "*",
                "returnGeometry": "false",
                "f": "json",
            }

            async with httpx.AsyncClient() as client:
                response = await client.get(
                    self.SECONDARY_PLAN_URL,
                    params=params,
                    timeout=15.0,
                    headers={"User-Agent": "LocationOverview/1.0"},
                )

                if response.status_code == 200:
                    data = response.json()
                    features = data.get("features", [])

                    if features:
                        attrs = features[0].get("attributes", {})
                        plan_name = attrs.get("PLAN_NAME") or attrs.get("NAME")

                        return {
                            "plan_name": plan_name,
                            "plan_number": attrs.get("PLAN_NO"),
                            "policies": [],  # Would require PDF extraction
                        }

        except Exception:
            pass

        return None

    async def _query_ward(self, lat: float, lon: float) -> Optional[Dict[str, Any]]:
        """Query Mississauga ward boundaries."""
        try:
            params = {
                "geometry": f"{lon},{lat}",
                "geometryType": "esriGeometryPoint",
                "inSR": "4326",
                "spatialRel": "esriSpatialRelIntersects",
                "outFields": "*",
                "returnGeometry": "false",
                "f": "json",
            }

            async with httpx.AsyncClient() as client:
                response = await client.get(
                    self.WARDS_URL,
                    params=params,
                    timeout=15.0,
                    headers={"User-Agent": "LocationOverview/1.0"},
                )

                if response.status_code == 200:
                    data = response.json()
                    features = data.get("features", [])

                    if features:
                        attrs = features[0].get("attributes", {})
                        return {
                            "ward_number": attrs.get("WARD") or attrs.get("WARD_NO"),
                            "ward_name": attrs.get("WARD_NAME") or attrs.get("NAME"),
                        }

        except Exception:
            pass

        return None

    async def _query_neighbourhood(
        self,
        lat: float,
        lon: float,
    ) -> Optional[Dict[str, Any]]:
        """Query Mississauga neighbourhood boundaries."""
        try:
            params = {
                "geometry": f"{lon},{lat}",
                "geometryType": "esriGeometryPoint",
                "inSR": "4326",
                "spatialRel": "esriSpatialRelIntersects",
                "outFields": "*",
                "returnGeometry": "false",
                "f": "json",
            }

            async with httpx.AsyncClient() as client:
                response = await client.get(
                    self.NEIGHBOURHOODS_URL,
                    params=params,
                    timeout=15.0,
                    headers={"User-Agent": "LocationOverview/1.0"},
                )

                if response.status_code == 200:
                    data = response.json()
                    features = data.get("features", [])

                    if features:
                        attrs = features[0].get("attributes", {})
                        return {
                            "name": attrs.get("NEIGHBOURHOOD") or attrs.get("NAME"),
                            "id": attrs.get("NEIGH_ID") or attrs.get("OBJECTID"),
                        }

        except Exception:
            pass

        return None

    def _categorize_zone(self, zone_code: Optional[str]) -> str:
        """Map zone code to category."""
        if not zone_code:
            return "Unknown"

        # Extract base zone from code (e.g., "R1" -> "R", "RM2" -> "RM")
        base = ""
        for char in zone_code.upper():
            if char.isalpha():
                base += char
            else:
                break

        return self.ZONE_CATEGORIES.get(base, "Mixed Use")

    def _get_permitted_uses(self, zone_code: Optional[str]) -> List[str]:
        """Get permitted uses for zone code."""
        if not zone_code:
            return []

        category = self._categorize_zone(zone_code)

        # Common permitted uses by category
        uses_by_category = {
            "Residential": ["Single-detached dwelling", "Home occupation"],
            "Residential Multiple": [
                "Multiple dwelling",
                "Semi-detached dwelling",
                "Townhouse",
            ],
            "Residential Apartment": ["Apartment building", "Retirement home"],
            "Commercial": [
                "Retail store",
                "Office",
                "Restaurant",
                "Personal service establishment",
            ],
            "City Centre Commercial": [
                "Mixed-use building",
                "Hotel",
                "Office",
                "Retail",
                "Residential",
            ],
            "Employment": [
                "Manufacturing",
                "Warehouse",
                "Office",
                "Research and development",
            ],
            "Institutional": ["School", "Hospital", "Place of worship", "Government office"],
            "Open Space": ["Park", "Recreation facility", "Conservation area"],
            "Greenlands": ["Conservation", "Natural heritage protection"],
        }

        return uses_by_category.get(category, ["See zoning by-law for permitted uses"])

    def _get_op_policies(self, designation: Optional[str]) -> List[str]:
        """Get Official Plan policies for designation."""
        if not designation:
            return []

        # Key policy themes by designation type
        designation_lower = designation.lower() if designation else ""

        if "residential" in designation_lower:
            return [
                "Protect stable residential neighbourhoods",
                "Direct intensification to appropriate areas",
            ]
        elif "employment" in designation_lower:
            return [
                "Protect employment lands from conversion",
                "Support economic development",
            ]
        elif "commercial" in designation_lower:
            return [
                "Support commercial development",
                "Encourage mixed-use where appropriate",
            ]
        elif "downtown" in designation_lower or "city centre" in designation_lower:
            return [
                "Encourage high-density mixed-use development",
                "Support transit-oriented development",
            ]

        return []

    def is_applicable(self, municipality: str) -> bool:
        """
        Check if this provider is applicable for the municipality.

        Args:
            municipality: Municipality name

        Returns:
            True for Mississauga
        """
        return municipality.lower() in [
            "mississauga",
            "city of mississauga",
        ]
